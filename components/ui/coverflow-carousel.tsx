...
export interface CoverflowSlide {
  ...
  /** Optional accent color for this slide's vote button (hex/oklch/etc) */
  accentColor?: string;
  /** Optional text color for the vote button */
  accentText?: string;
}
...
export interface CoverflowCarouselProps {
  ...
  /** Auto-advance the carousel. Pass `true` for the default 3 s interval or a
   *  number (ms) to customise it. Set to `false` / omit to disable. */
  autoPlay?: boolean | number;
  /** Fired when the active slide changes. */
  onChange?: (index: number) => void;
  /** If provided, shows a vote button under the carousel. Called with current slide index. */
  onVote?: (index: number) => void;
  /** Label for the vote button. Defaults to "Voter pour {title}" */
  voteLabel?: string;
}
...
export function CoverflowCarousel({
  ...
  /** True while the user is hovering or dragging — suppresses auto-advance. */
  ...
  // Auto-play: advance by one slide at the chosen interval, unless paused.
  React.useEffect(() => {
    if (!autoPlay) return;
  ...
  // Fire onChange event when selection changes
  React.useEffect(() => {
    onChange?.(selected);
  }, [selected, onChange]);
  ...
  return (
    ...
    >
      ...
      {/* Vote button — appears for the current centred candidate */}
      {onVote && active && (
        <div
          key={`vote-btn-${selected}`}
          className="mt-8 flex flex-col items-center gap-3 animate-in fade-in slide-in-from-bottom-2 duration-300"
        >
          <button
            type="button"
            onClick={() => onVote(selected)}
            className="relative overflow-hidden group flex items-center gap-3 rounded-full px-10 py-4 font-bold text-sm uppercase tracking-widest transition-all duration-300 shadow-2xl"
            style={{
              background: active.accentColor ?? "#e8c26a",
              color: active.accentText ?? "#101014",
              boxShadow: `0 8px 32px -8px ${(active.accentColor ?? "#e8c26a")}60`,
            }}
            aria-label={voteLabel ?? (active.title ? `Voter pour ${active.title}` : "Voter")}
          >
            {/* Shimmer overlay */}
            <span
            ...
            <span>
              {voteLabel ?? (active.title ? `Voter pour ${active.title}` : "Voter")}
            </span>
          </button>
          {active.subtitle && (
            <p className="text-xs tracking-wider" style={{ color: "rgba(255,255,255,0.45)" }}>
              {active.subtitle}
            </p>
          )}
        </div>
